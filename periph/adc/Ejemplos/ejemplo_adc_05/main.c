...
int main(void) {
    ...
    /* Configurar el Timer1 para contar 10 milisegundos */
    Timer1_Initialize();
    
    while(1)
    {
        /* Si ha transcurrido 10ms ... */
        if(IFS0bits.T1IF == 1)
        {
            /* Limpiar la bandera del Timer1 */
            IFS0bits.T1IF = 0;
            /* Iniciar el muestreo, la conversión iniciará de forma automática
...
void ADC_Initialize(void)
{
    ...
    my_adc.conversion_trigger = ADC_INTERNAL_COUNTER_STARTS_CONVERSION; // Conversión activada por contador interno
...
void Timer1_Initialize(void)
{
    ...
    timer1.prescaler = TIMER_PRESCALER_1;
